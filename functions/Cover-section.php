...
function sectionCover(
  ...
  if ($videoFile) {
    ...
    $out .= "<div class='sectionCover-video position-relative overflow-hidden min-vh-40 min-vh-sm-90 bg-lighter rounded-4' data-aos='fade-up' data-aos-duration='1600'>";
      $out .= "<video class='position-absolute w-100 h-100 top-0 left-0 object-fit-cover' width='1920' height='1080' playsinline autoplay muted loop $poster><source src='{$videoFile->url}' type='video/mp4'></video>";
    $out .= "</div>";
  } else {
    // Images
    if (in_array('filipok.koriphey.ru', $config->httpHosts)) {
      // Filipok site specific parameters
      $out .= imageSlider($imagesMain, "sectionCover-image min-vh-40 min-vh-sm-60 rounded-4", null, 900, "position-absolute w-100 h-100 top-0 left-0 object-fit-scale");
    } else {
      // Default parameters for other sites
      $out .= imageSlider($imagesMain, "sectionCover-image min-vh-60 min-vh-sm-80 overflow-hidden rounded-4");
    }
  }
  ...
  if (!pages("/settings/")->options_site->has(3)) {
    $out .= "<div class='sectionCover-news container-xxl mt-4 mt-lg-6'>";
