  ...
  if (isset($settingsPage) && !$settingsPage->check_off) {
    ...
    $out = "<div class='CTA-row sectionContent shadow bg-white border rounded mt-7' data-aos='zoom-in' edit='$edit.images_main,text_1,text_summary,link,text_2,check_off'>";
      $out .= "<div class='row align-items-center'>";
        ...
        $out .= "</div>";
        $out .= "<div class='col-12 col-lg-7 text-center text-lg-start pb-4 pb-lg-0'>";
          $out .= "<div class='px-3 px-lg-4 py-4 py-lg-7 max-w-xxl-80'>";
            $out .= "<h2 class='display-2 text-primary-dark' data-aos='zoom-in' data-aos-delay='600'>$title</h2>";
            $out .= "<div class='bg-chevron fs-lg-3 txt markerlist-icon' data-aos='zoom-in' data-aos-delay='800'>";
              $out .= $summary;
            $out .= "</div>";
            $out .= "<span class='button-group d-block py-2'><a class='btn btn-lg btn-secondary my-3 mt-lg-4' href='$link'>$linkCaption</a></span>";
          $out .= "</div>";
        $out .= "</div>";
      $out .= "</div>";
    $out .= "</div>";
