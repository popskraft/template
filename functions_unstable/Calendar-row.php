...
function calendar($headlineClass = "")
{
  ...
  foreach ($groupedEvents as $yearMonth => $eventsInGroup) {
    ...
    $out .= "<div class='calendar-row-divider divider border-top'></div>";
    $out .= "<div class='calendar calendar-row row $headlineClass'>";
      $out .= "<div class='calendar-row-month-col col-lg-5 pt-4'>";
        // Apply the season class dynamically
        $out .= "<div class='calendar-row-month-year month-year display-4 text-lg-end $seasonClass'>$monthRusWithYear</div>";
      $out .= "</div>";

      $out .= "<div class='calendar-row-events-col col-lg-7 py-4 border-start-lg'>";
      ...
      foreach ($eventsInGroup as $event) {
        ...
        $out .= "<div class='calendar-row-event-item calendar-data-item d-flex flex-column flex-sm-row flex-wrap align-items-start mb-2 ps-3'>";
        $out .= $formattedDate;
        if ($eventTextSummary || $eventText) {
          $out .= "<a class='calendar-row-event-link link fw-bold fs-xl-3' href='$eventUrl'>$eventName</a>";
        } else {
          $out .= "<span class='calendar-row-event-name fs-xl-3 fw-bold text-dark'>$eventName</span>";
        }
        $out .= "</div>";
      }
