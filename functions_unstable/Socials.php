    ...
    foreach ($socials as $socUrl) {
      foreach ($brands as $key => $iconName) {
        if (strpos($socUrl, $key)) { 
          $out .= "<div class='socials-item d-flex align-items-center'>";
            $out .= iconCaption($socUrl, $caption, $iconName, $linkClass, $iconClass, $iconSize);
            $out .= "<a class='socials-link $linkClass' rel='nofollow' title='$iconName' href='$socUrl' target='_blank'>$iconName</a>";
          $out .= "</div>";
        }
      }
    }
